(ns airtable.main
  "Kotodama WASM entrypoint for the Airtable clean-room actor (L4) — Clojure port.
  ...
  facts (`airtable.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "airtable")
(def tier "L4")
...
(defn page-limit
  ...
  src/airtable/page_limit.kotoba."
  [requested]
  (if (pos? requested)
...
(def entity-specs
  [{:entity "Base"    :plural "bases"    :id-prefix "airtable_bas"
    :fields [:name :permissionLevel]                    :required [:name :permissionLevel]
    :coerce {}                       :refs {}}
   {:entity "Table"   :plural "tables"   :id-prefix "airtable_tab"
    :fields [:name :description :primaryFieldId]        :required [:name :description]
    :coerce {}                       :refs {}}
   {:entity "Field"   :plural "fields"   :id-prefix "airtable_fie"
    :fields [:name :type :description]                  :required [:name :type]
    :coerce {}                       :refs {}}
   {:entity "Record"  :plural "records"  :id-prefix "airtable_rec"
    :fields [:createdTime :fieldsJson]                  :required [:createdTime :fieldsJson]
    :coerce {}                       :refs {}}
   {:entity "View"    :plural "views"    :id-prefix "airtable_vie"
    :fields [:name :type :personalForUserId]            :required [:name :type]
    :coerce {}                       :refs {}}
   {:entity "Comment" :plural "comments" :id-prefix "airtable_com"
    :fields [:text :lastUpdatedTime]                    :required [:text :lastUpdatedTime]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"airtable.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "airtable-compat" :tier tier :entities entities} 200])
...
(defn start! [] :airtable-compat/ready)
